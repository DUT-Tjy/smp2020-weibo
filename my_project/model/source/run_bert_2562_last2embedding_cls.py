...
import json
from pytorch_transformers.modeling_bert import BertForSequenceClassification_last2embedding_cls, BertConfig
...
logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                    ...
                    level = logging.INFO)
MODEL_CLASSES = {
    'bert': (BertConfig, BertForSequenceClassification_last2embedding_cls, BertTokenizer),
}
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, split_num,
    ...
    features = []
    for example_index, example in enumerate(examples):
        context_tokens = tokenizer.tokenize(str(example.text_a))
        ...
        for i in range(split_num):
            ...
            _truncate_seq_pair(context_tokens_choice, max_seq_length - 3, i == index_end)
...
def _truncate_seq_pair(tokens_a, max_length, last_index):
...
def accuracy(out, labels):
    ...
    return f1_score(labels,outputs,labels=[0,1,2,3,4,5],average='macro')
...
def main():
    ...
    parser.add_argument("--freeze", default=0, type=int, required=False,
                        help="freeze bert.")
    parser.add_argument("--not_do_eval_steps", default=0.5, type=float,
    ...
    # Prepare model
    model = BertForSequenceClassification_last2embedding_cls.from_pretrained(args.model_name_or_path,args,config=config)
    ...
    if args.do_test:
        ...
        gc.collect()
        args.do_train=False
        model = BertForSequenceClassification_last2embedding_cls.from_pretrained(os.path.join(args.output_dir, "pytorch_model.bin"),args,config=config)
        if args.fp16:
            model.half()
        model.to(device)
        if args.local_rank != -1:
        ...
        for file,flag in [('dev.csv','dev'),('test.csv','test')]:
            ...
            if flag=='test':
                ...
                df['label_4']=logits[:,4]
                df['label_5']=logits[:,5]
                df[['id','label_0','label_1','label_2','label_3','label_4','label_5']].to_csv(os.path.join(args.output_dir, "sub.csv"),index=False)
            if flag == 'dev':
                df = pd.read_csv(os.path.join(args.data_dir, file))
                df['label_0']=logits[:,0]
                df['label_1']=logits[:,1]
                ...
                df['label_4']=logits[:,4]
                df['label_5']=logits[:,5]
                df[['id', 'label_0', 'label_1','label_2', 'label_3','label_4', 'label_5']].to_csv(os.path.join(args.output_dir, "sub_dev.csv"),index=False)
